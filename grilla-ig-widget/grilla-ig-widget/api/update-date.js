...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const dateEntries = Object.entries(dbJson.properties || {}).filter(([, p]) => p.type === 'date');
    // si hay varias propiedades de fecha (ej. "Fecha de inicio" y "Fecha de
    // publicación"), preferimos la de publicación — es la que ordena la grilla.
    const dateEntry = dateEntries.find(([name]) => /public/i.test(name)) || dateEntries[0];
    if (!dateEntry) {
